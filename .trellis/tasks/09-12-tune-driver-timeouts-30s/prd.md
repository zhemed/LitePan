...
- [ ] 115 上传响应头兜底已是 30s；189 上传响应头兜底已是 30s；189 API 仍 30s
- [ ] 115 侧新增/更新测试锁定三个值；`go test ./drivers/115_Open/ ./internal/httpx/` 通过
- [ ] `go vet ./...`、`go test ./...`、`go build ./...` 全绿；web 三连（type-check/build/check:memo）全绿
- [ ] 版本号 `0.0.39` 已更新（`README.md` + `docker-compose.yml`）
- [ ] 镜像 `ghcr.io/zhemed/litepan:0.0.39`（含 `:v0.0.39`、`:latest`）三 tag 同 digest 已推送
- [ ] `git tag v0.0.39` 已推送，GitHub release `v0.0.39` 已创建
- [ ] 本地容器已重建到 0.0.39，health / form 表单登录 / 上传任务汇总三连通过
- [ ] 已知取舍（115 慢 API 30s 失败的症状与回退方式）已记录在 PRD/Journal
